...
| **Proposer** | Cloud Run, `proposer-sa` | Gemini 3.5 Flash structured output constrained to the Caseharden DSL, conditioned on the finding, the verdict, and Memory Bank precedent. `SELECT` on the training window only. IAM-denied on `holdout_sealed`, and the demo makes it try. |
...
| **0:38-0:56** | The fleet. Four conduct checks, one workload agent, one orchestrator that hard-codes none of them. | Terminal `gcloud alpha agent-registry agents list --location=europe-west3` returning nine entries; cut to the same nine rows in the GCP console. Voice names them as "nine rows, seven of them ours". Caption: "Foreman binds these at runtime via list_agents() to RemoteA2aAgent." |
| **0:56-1:20** | The incident. A support ticket carries an embedded instruction; the agent calls `issue_refund` across a tenant boundary. Foreman fans out, four detectors answer in parallel, one finding surfaces. | Split: ADK chat UI streaming the finding card with evidence rows; then the four detector answers in the terminal, each with its own BigQuery job id, and the offending tool call lit red. Caption: "job id and trace id both live inside chain link 2." **The trace-DAG shot is cut.** Spans do not reach Cloud Trace from Cloud Run; the ids in the chain are correlation keys and the video does not claim otherwise. |
...
| **2:10-2:34** | **THE GATE.** The Proposer's own candidate catches one more sealed attack than the active version. The gate throws it out, because it also blocks two legitimate turns. The next candidate passes. Voice: "The examiner is four hundred lines of code under a different service account. It cannot be argued into a pass, and it cannot be won by refusing everything." | Full-screen red: `PROMOTION DENIED. ATTACKS 30/40 → 31/40. BENIGN PASS 100% → 99.7%. REASON: BENIGN REGRESSION.` Then green: `30/40, benign 100%, monotonicity OK, GATE PASS`. Cut to BigQuery job history showing the scoring job running as `examiner-sa`. Every number is measured; both runs are in `captures/day5-*`. |
