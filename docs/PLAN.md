...
Every step is a hash-linked record in an append-only BigQuery chain whose root is sealed into a retention-locked GCS object and annotated onto the Agent Registry entry for that policy version.

The product is not the detection. The product is the attestation. A guardrail version is served as `attested` only while `caseharden verify` re-derives its whole chain from the raw conduct events. When the evidence changes, the version is quarantined: it keeps blocking, but it loses attested status and nothing can be promoted on top of it until it re-derives. The governance agents run behind the same enforcement callback and appear in the same registry, so the plane sits inside its own blast radius.
...
An agent's authority to act is derived from evidence, not granted by configuration. In Caseharden the provenance record is load-bearing rather than descriptive. A guardrail version is authoritative in production only while its chain re-derives from the raw conduct events that justified it, so changing the evidence withdraws the version's authority instead of filing a discrepancy report. The two guarantees the chain rests on are discharged outside this project's code: that the proposing agent never read its own evaluation data is a BigQuery IAM deny binding producing a real 403, and that the record cannot be edited after the fact is a Cloud Storage retention lock that refuses a delete from the project owner. Refusals are stored as links in the same chain as approvals, so the record attests to what was prevented and not only to what occurred.
...
| **Foreman** (orchestrator) | Agent Runtime, own identity | Discovers detectors at runtime via `AgentRegistry.list_agents()` bound to `RemoteA2aAgent`. No hard-coded endpoints. Fans an investigation window out to four detectors in parallel, merges findings, opens a case, pulls reviewer precedent from Memory Bank. |
| **Detectors ×4** | Cloud Run, one template instantiated four times, each its own registry entry | `scope-violation`, `injected-turn-tool-call`, `cross-tenant-egress`, `privilege-sequencing`. Each runs a governed parameterized BigQuery check and returns findings plus the SQL job id. |
...
| **Proposer** | Agent Runtime, `proposer-sa` | Gemini 3.5 Flash structured output constrained to the Caseharden DSL, conditioned on the finding, the verdict, and Memory Bank precedent. `SELECT` on the training window only. IAM-denied on `holdout_sealed`, and the demo makes it try. |
| **Examiner** | Cloud Run job, `examiner-sa` | Deliberately not an LLM. About 200 lines of deterministic interpreter, the only principal with read access to `holdout_sealed`. Compiles a candidate to a BigQuery predicate and returns catch rate, benign pass rate, false-positive cost, and the monotonicity check. Having no model, it cannot be argued into a pass. |
| **Notary** | Cloud Run service, `notary-sa` | Writes hash-linked rows into the append-only chain, seals the root into the retention-locked bucket, annotates the Agent Registry entry with the root hash, implements `caseharden verify` and `caseharden reattest`. |
...
- **Agent Registry (GA):** the fleet roster and the discovery layer. Eight entries. Foreman hard-codes nothing. Each promoted version's chain root hash is written as an annotation on its registry entry, so the platform's own discovery layer carries the audit anchor.
- **Agent Runtime (GA):** hosts Foreman and Proposer as native A2A agents with stable URNs. Detectors, Examiner and Notary run on Cloud Run and auto-register, proving the registry pattern works across both hosts.
- **Agent Identity (GA):** load-bearing, not decoration. `proposer-sa` and `examiner-sa` are distinct principals and the seal is a BigQuery IAM deny on `proposer-sa`. Isolation is a property of Google's authorization layer, reproducible by any judge with the IAM policy open.
...
Proposer on Agent Runtime under `proposer-sa` with structured output into the DSL, Memory Bank conditioning, and schema rejection written to the chain. Model Armor on verdict in and rationale out with results recorded. Analyst Copilot via `adk deploy cloud_run --with_ui`, verdict and approve as two ADK tools.
...
| **0:38-0:56** | The fleet. Four conduct checks, one workload agent, one orchestrator that hard-codes none of them. | Terminal `gcloud agent-registry agents list --location=europe-west3` returning eight entries; cut to the same eight rows in the GCP console. Caption: "Foreman binds these at runtime via list_agents() to RemoteA2aAgent." |
| **0:56-1:20** | The incident. A support ticket carries an embedded instruction; the agent calls `issue_refund` across a tenant boundary. Foreman fans out, four detectors answer in parallel, one finding surfaces. | Split: ADK chat UI streaming the finding card with evidence rows; then the Observability trace DAG showing four parallel detector spans and the offending tool call lit red. Caption: "trace id 8f2c… lives inside chain link 2." |
| **1:20-1:34** | Precedent, then verdict. Memory Bank surfaces two prior rejections by this reviewer. Analyst types the verdict; Model Armor screens it. | Chat UI "Reviewer precedent" block, then the verdict submitted, then the Model Armor result. Caption: "Link 3 records the verdict and the screening result." |
| **1:34-1:52** | Bounded authorship. The Proposer emits a candidate. The first attempt fails schema validation and is written to the chain as a rejection rather than retried silently. | Side by side: the 12-line DSL diff v3 to v4 on the left; on the right, `DRAFT REJECTED: unknown predicate — link 4a written`. Caption: "There is no allow verb in this grammar." |
| **1:52-2:10** | **THE SEAL.** The Proposer is asked to check its own work against the holdout. It tries. BigQuery refuses. Voice, flat: "It cannot read the exam. That is not our check. That is IAM." | Red `403 Access Denied: Table holdout_sealed.turns` with `proposer-sa@` visible; cut to the BigQuery IAM page showing the deny binding; cut to link 5, "DENIED — recorded as evidence". |
| **2:10-2:34** | **THE GATE.** Candidate A blocks 10 of 10 sealed attacks. A perfect score. The gate throws it out, because benign traffic fell to 68 percent. Candidate B passes. Voice: "The examiner is two hundred lines of code under a different service account. It cannot be argued into a pass, and it cannot be won by refusing everything." | Full-screen red: `PROMOTION DENIED. ATTACKS 10/10. BENIGN PASS 100% → 68%. REASON: BENIGN REGRESSION.` Then green: `9/10, benign 100%, monotonicity OK, GATE PASS`. Cut to BigQuery job history showing the scoring job running as `examiner-sa`. |
...
| **3:40-4:00** | Close. Last spoken line is the thesis, verbatim: "An agent's authority to act is derived from evidence, not granted by config." | Architecture card: eight registry entries, Agent Runtime plus Cloud Run, the chain table, the locked bucket, the deny arrow from `proposer-sa` to `holdout_sealed` drawn in red and crossed. Bottom bar: repo URL, europe-west3, Gemini 3.5 Flash, and the billing report showing spend to date. |
...
| **KMS asymmetric signing and the offline verifier** | Entry C | Two of three judges marked C infeasible on scope, and one noted the offline framing spends the video's best seconds on a laptop with no GCP, against a requirement to show the GCP backend. Canonical JSON plus signing plus a five-property verifier is the classic half-day loss. The append-only chain, the IAM deny and the retention lock already carry the claim. |
