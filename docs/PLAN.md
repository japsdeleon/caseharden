...
- **Agent Identity (GA):** load-bearing, not decoration. `proposer-sa` and `examiner-sa` are distinct principals. No principal in the project holds a project-wide BigQuery table read, `holdout_sealed` has its inherited project-wide entries stripped, and exactly one service account is granted it. Isolation is a property of Google's authorization layer, reproducible by any judge with the dataset's access list open, and the Cloud Audit entry names `bigquery.tables.getData` as denied.
